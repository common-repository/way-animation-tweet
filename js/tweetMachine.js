		...
		$(this).each(function () {
			...
			if (this.tweetMachine) {
				...
				settings = $.extend({
					...
					user_name:		'tilesstore', // Set your username
					include_retweets: true,
                    favorited:      true,
                    favorite_count: true,
                    geo_loc: '22.2734869,-70.8212963,20km',
                    geo_enabled:    true,
     
                                 // Set to true or false if you want to include retweets
				...
				this.tweetMachine = {
					...
					buildTweet: function (tweet) {
                        ...
                        tweetObj.find('.avatar').attr('class','avatar '+tweetMachine.settings.avatarShape);
                        // Set the username
                        ...
                        // Set the reaply
                        tweetObj.find('.reaply')
                            .attr('href',"https://twitter.com/intent/tweet?in_reply_to="+tweet.id_str)
                            .attr('target','_blank');
                        // Set the retweet
                        tweetObj.find('.retweet')
                            .attr('href',"https://twitter.com/intent/retweet?tweet_id="+tweet.id_str)
                            .attr('target','_blank');
                        // Set the favorite
                        tweetObj.find('.favorite')
                            .attr('href',"https://twitter.com/intent/favorite?tweet_id="+tweet.id_str)
                            .attr('target','_blank');    
                            
                        // Set the Retweetcount
                        tweetObj.find('.retweetcount')
                            .attr('href',"https://twitter.com/"+tweet.user.screen_name+"/statuses/"+tweet.id_str)
                            .attr('target','_blank')  
                            .html(tweet.retweet_count+" Retweets"); 
                         // https://twitter.com/BarackObama/statuses/266031293945503744  
                         // Set the favoritescount
                        tweetObj.find('.favouritescount')
                            .attr('href',"https://twitter.com/"+tweet.user.screen_name+"/statuses/"+tweet.id_str)
                            .attr('target','_blank')                         
                            .html(tweet.favorite_count+" Favorites"); 
